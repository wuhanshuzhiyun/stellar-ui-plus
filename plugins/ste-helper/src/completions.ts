...
export function registerCompletions(context: ExtensionContext) {
    ...
    const attrProvider: CompletionItemProvider = {
        provideCompletionItems(document: TextDocument, position: Position) {
            ...
            if (nextCharacter !== ' ' && nextCharacter !== '\n' && nextCharacter !== '/' && nextCharacter !== '>') return null;

            if (!Array.from(text.matchAll(ATTR_RE)).length) return null;
            ...
            if (currentIndex > endIndex || currentIndex < startIndex!) return null;
