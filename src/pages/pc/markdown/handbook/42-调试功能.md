...
由于 Vue 3 版本编译到微信开发者工具后，在 `appData` 工具栏中看不到页面变量值，所以提供了此工具，可在控制台显示页面的双向绑定值。
...
在项目的 `vite.config.js` 或 `vite.config.ts` 中添加以下代码：
...
**说明：** 插件默认会给 `src/pages/` 目录下的 vue 文件添加此功能，若项目目录不在此，可通过 `autoDebugPlugin(路径)` 自定义目录。

### 步骤 2：初始化调试工具

在项目入口 `main.js` 或 `main.ts` 中：
