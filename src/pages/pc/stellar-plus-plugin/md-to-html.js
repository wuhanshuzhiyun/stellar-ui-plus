...
function splitMarkdown(content) {
    try {
        ...
        if (parts.length < 4) {
            ...
            console.error(filePath, '文件格式错误,请按照正确格式，用---$分割 README.md 文件');
            return {
                desc: content,
                demo: '',
                api: '',
...
module.exports = function md2HtmlPlugin() {
    ...
    return {
        ...
        async transform(code, id) {
            ...
            if (!shouldSplitFile(id)) return await originalPlugin.transform.call(this, code, id);

            // 组件预览页面的md文件需要分割
            const sections = splitMarkdown(code, id);
            ...
            for (const [key, content] of Object.entries(sections)) {
                try {
                    ...
                    if (result?.code) processedSections[key] = extractHtmlFromCode(result.code);
                } catch (error) {
