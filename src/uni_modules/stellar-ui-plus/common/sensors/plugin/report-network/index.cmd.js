...
function pick(obj, keys) {
  return obj == null ? {} : keys.reduce((result, key) => {
...
var _ = {
  ...
  ReportNetWork = {
    ...
    reportNetWork(o) {
      ...
      if(o.collect_url_path && !isArray(o.collect_url_path) || o.ignore_http_code && !isArray(o.ignore_http_code) || o.collect_header_attribute && !isArray(o.collect_header_attribute)){
        _.log("请传入正确的数组对象！")
        return false;
      }
        ...
        invoke: (options) => {
          ...
          const requestEntry = {
            options: {
              ...
              query: options.query,
              data: options.data,
              url: options.query ? `${options.url}?${Object.keys(options.query).map(key => {
        ...
        complete: (res) => {
          ...
          if (requestEntry) {
            ...
            const data = {
              ...
              interface_trace_id: res.header && res.header['x-trace-id'] ? res.header['x-trace-id'] : para.error_text,
            }
            // 排除采集接口路径地址
