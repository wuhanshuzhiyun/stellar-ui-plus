...
export default function useData({
    ...
    const cmpViewValue = computed(() => {
        ...
        confirmValue.value.forEach((value, index) => {
            let item = null;
            if (props.mode === 'tree') item = utils.findTreeNode(props.list, value, props.valueKey, props.childrenKey);
            else if (props.multiple && dataOptions.value.length === 1) item = dataOptions.value[0]?.find(item => item[props.valueKey] === value);
    ...
    const initOptions = () => {
        ...
        if (list[0] && !Array.isArray(list[0])) list = [list];
    ...
    watch(
        ...
        v => {
            if (Array.isArray(v)) {
                ...
                const value: string[] = v ? utils.dayjs(v).format('YYYY MM DD HH mm ss').split(' ') : [];
                setConfirmValue(value.map(item => Number(item)));
            } else {
                if (dataOptions.value.length > 1 || props.multiple) console.error('ste-select: value必须为数组（单列单选模式value可以为string或number类型）');
    ...
    const onUserFilterable = (e?: any) => {
        ...
        setFilterableTime(() => {
            ...
            if (e) emits('inputFilterable', value);
    ...
    const onFocus = () => {
        if (!cmpAutoFilterable.value) return;
        setInputView('');
        const v = confirmValue.value;
        const value = dataOptions.value[0]?.find(item => item[props.valueKey] === v[0]);
    ...
    const openOptions = async () => {
        ...
        if (selected.value.length < dataOptions.value.length) {
            ...
            if (dataOptions.value.length > 1) selected = initSelected(selected);
        ...
        if (y === 'auto') {
            y = 'bottom';
            if (boundary.top - boundary.bottom > 0) y = 'top';
        }

        if (x === 'auto') {
            x = 'start';
            if (boundary.right - boundary.left < 0) x = 'end';
        }
    ...
    const onSelect = (col: number, item: SelectOption, isAllowCreate = false) => {
        ...
        onConfirm();
        if (!props.multiple) nextTick(onCancel);
    };

    const active = (index: number, item: SelectOption) => {
        if (dataOptions.value.length > 1) return selected.value[index] === item[props.valueKey];
