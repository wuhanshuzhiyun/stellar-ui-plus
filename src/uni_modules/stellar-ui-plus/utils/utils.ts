...
const utils = {
    ...
    formatPx<T extends PxType>(value: number | string, restype?: T): PX<T> {
        ...
        if (typeof format === 'string' && utils.isNaN(format)) {
            if (/^\d+px$/i.test(format)) return (restype === 'num' ? Number(format.slice(0, -2)) : format) as PX<T>;
            else if (/^\d+rpx/i.test(format)) format = format.slice(0, -3);
            else return format as PX<T>;
        }
        let px = 0;
        if (format !== 0) px = (Number(format) * System.getWindowWidth()) / 750;
    ...
    fenToYuan(val: any, digits: number = -1, defaultVal: string = '', part: PartType = 0): string | undefined {
        ...
        if (part === 0) {
            ...
            if (newVal.split('.').length > 1) return `.${newVal.split('.')[1]}`;
            else return '';
        }
    },
    /**
   ...
   */
        ...
        for (const key in source) {
            if (Object.prototype.hasOwnProperty.call(source, key)) {
                ...
                if (isObject(sourceValue)) {
                    if (!isObject(target[key as unknown as keyof T])) (target as any)[key] = {};
    ...
    addUnit(val: string | number) {
        val = String(val);
        let newVal;
        if (this.isNumber(val)) newVal = `${val}rpx`;
        else newVal = val;

        // #ifdef H5
        if (newVal && newVal.includes('rpx')) newVal = this.formatPx(newVal);
    ...
    getMediaFileType(filePath: string | undefined, compatible = 1) {
        ...
        if (videoType.includes(type)) return 'video';

        if (audioType.includes(type)) return 'audio';

        if (imageType.includes(type)) return 'image';
    ...
    scrollViewX({
        ...
        if (left < boxLeft) return (scrollLeft += left - boxLeft);

        if (right > boxRight) return (scrollLeft += right - boxRight);
    ...
    deepEqual(obj1: { [key: string]: any }, obj2: { [key: string]: any }, ignoreKeys: any[] = []): boolean {
        if (obj1 === obj2 || (this.isEmpty(obj1) && this.isEmpty(obj2))) return true; // 简单类型相等或引用相等

        if (obj1 == null || obj2 == null || typeof obj1 !== 'object' || typeof obj2 !== 'object') return false; // 其中一个为 null，或者不是对象
        ...
        if (keys1.length !== keys2.length) return false; // 忽略指定 key 后，对象属性数量不相等

        for (const key of keys1) {
            if (!Object.prototype.hasOwnProperty.call(obj2, key)) return false; // obj2 没有 obj1 的属性

            if (!this.deepEqual(obj1[key], obj2[key], ignoreKeys)) return false; // 递归比较子对象，忽略指定 key
        }
    ...
    formatTree(tree: TreeNode[], options: FormatTreeOptions = {}): TreeNode[] {
        ...
        const _formatTree = (tree: TreeNode[], parentNode: string | number, depth: number): TreeNode[] => {
            return tree.map(item => {
                if (item[childrenKey] && item[childrenKey].length) Object.assign(item, { [childrenKey]: _formatTree(item[childrenKey], item[valueKey], depth + 1) });

                let _otherAttributes: Record<string, any>;
                if (typeof otherAttributes === 'function') _otherAttributes = otherAttributes(item);
    ...
    filterTree(tree: TreeNode[], filterFunc: (node: TreeNode) => boolean, valueKey = 'value', childrenKey = 'children') {
        ...
        const _flatten = (tree: TreeNode[]) => {
            ...
            tree.forEach(n => {
                ...
                if (nodeValues.includes(node[valueKey])) result.push(node);

                if (node[childrenKey] && node[childrenKey].length) Object.assign(node, { [childrenKey]: _flatten(node[childrenKey]) });
            });
            return result;
        };
        return _flatten(tree);
    ...
    richTextTagAddStyle(html: string, tag: string, checkProperties: string[] | string, replaceProperty: string) {
        ...
        if (Object.prototype.toString.call(checkProperties) === '[object String]') checkProperties = [checkProperties as string];
    ...
    treeToTable(tree: TreeNode[], values: (number | string)[] = [], valueKey = 'value', childrenKey = 'children') {
        const _flatten = (tree: TreeNode[], depth = 0) => {
            ...
            item = item || tree[0];
            if (item && item[childrenKey]) result.push(..._flatten(item[childrenKey], depth + 1));
